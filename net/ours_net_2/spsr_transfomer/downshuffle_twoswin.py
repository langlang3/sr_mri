...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=5, is_downshuffle=False,
        ...
        self.ln11 = nn.LayerNorm([120,120])
        self.ln12 = nn.LayerNorm([120,120])
        self.ln21 = nn.LayerNorm([120,120])
        self.ln22 = nn.LayerNorm([120,120])
        self.t1act = nn.GELU()
    def forward(self, x):
...
if __name__ == "__main__":
    net=SwinTransformer(dim=32, input_resolution=(60,60), num_heads=4,)
    a=torch.randn(4,32,60,60)
    out=net(a)
    print(out.shape)
