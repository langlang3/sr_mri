...
class SAM(nn.Module):
    def __init__(self, nf=32, use_residual=True, learnable=True):
        ...
        if self.learnable:
            self.conv_noshared = nn.Sequential(nn.Conv2d(nf , nf*16, 3, 1, 1, bias=True),
                                             nn.GELU())
    ...
    def forward(self, lr, ref_ori):#lr (b,32,60,60)  ref (b,32,240,240)
        b,c,h,w = lr.shape
