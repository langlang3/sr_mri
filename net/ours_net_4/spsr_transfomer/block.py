...
class shuffle_transformer(nn.Module):

    def __init__(self, nc, kernel_size=3, gc=32, stride=1, bias=True, pad_type='zero', \
            norm_type=None, act_type='gelu', mode='CNA'):
        super(shuffle_transformer, self).__init__()
        self.shuffle_transformer = ResidualDenseBlock_5C(nc, kernel_size, gc, stride, bias, pad_type, \
            norm_type, act_type, mode)
        self.shuffle_transformer = ResidualDenseBlock_5C(nc, kernel_size, gc, stride, bias, pad_type, \
            norm_type, act_type, mode)
        self.shuffle_transformer = ResidualDenseBlock_5C(nc, kernel_size, gc, stride, bias, pad_type, \
    ...
    def forward(self, x):
        out = self.dilate_net(x)
        out = self.dilate_net(out)
        out = self.dilate_net(out)
        return out.mul(0.2) + x
